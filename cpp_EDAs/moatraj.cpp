...
for(j=0;j<1000;j++)
   {   
    protein_inst = params[1]+j;
    ReadProtein(protein_inst,modeprotein);     
    cout<<"Alg : "<<ExperimentMode<<", number codifying bits : "<<length<<", n : "<<vars<<", psize : "<<psize<<", Trunc : "<<T<<", max-gen : "<<Maxgen<<", BestElit. : "<<BestElitism<<", NNeighbors  : "<<CliqMaxLength<<", MaxFun  : "<<Max<<", func : "<<func<<", params[0] : "<<params[0]<<", params[1] : "<<params[1]+j<<endl; 
