...
{  
   ...
   else if (LearningType==1  || LearningType==3) DestroyFDAOverl();
...
{   
    ...
    else if (LearningType==1  ||  LearningType==3) FDACallProb();	 
}   
...
{  
    int i; 
    //for (i=0;i<ncliques; i++)   SetOfCliques->ListCliques[OrderCliques[i]]->printmarg();
 if (Priors) 
 {
 for (i=0;i<ncliques; i++)   SetOfCliques->ListCliques[OrderCliques[i]]->Normalize(genepoollimit); //(actualpoolsize);  
...
{   
        ...
        else if(LearningType==3)  AncestralOrderingFactOnlyCliq(1); // MN-FDA legal factorization     
...
void DynFDA::GenPopProtein(int From, Popul* NewPop, HPProtein* FoldingProtein, double GenPrior)   
{    
   int i,j;   
...
//SetOfCliques->ListCliques[OrderCliques[i]]->Normalize(genepoollimit,GenPrior); //Modificacion Protein
	 ...
	 GenIndividualMNFDAProtein(NewPop,i,auxvectGen,legalconfGen,FoldingProtein);            
...
void DynFDA::GenIndividualMNFDAProtein(Popul* NewPop, int pos, int* auxvect, int* legalconf, HPProtein* FoldingProtein)   
{   
...
if (numberinst == 0)  
                      SetOfCliques->ListCliques[OrderCliques[i]]->Instantiate(auxvect);  		                  else if(numberinst <  SetOfCliques->ListCliques[OrderCliques[i]]->NumberVars)  
		       SetOfCliques->ListCliques[OrderCliques[i]]->PartialInstantiate(auxvect,legalconf);                                     
                 }  
                for (i=0;i<length; i++)  
                 {     
