...
{
 int k;
 for(k=eli; k < last; k ++)  epop->SetVal(k,MyIsing->evalfunc(epop->P[k]));
}
...
MaxNumCliq = 2000; // Maximum number of cliques for Markov 
...
//int auxpsize = 8192;
 ...
 if(func==100)  // Binary Ising Model 
     {     
      filedetails[0]=0; 
      MatrixFileName[0]=0; 
 ...
 while(psize<250000 && succ<5) // Experiments for critical pop-size
   {
  ...
  while(nsucc==0 && succ<5)  // Only for Experiments of critical pop-size  //while (i<cantexp) //&& nsucc<1
        { 
          NArchiveSols = 0;     
          currentexp = i;	  
