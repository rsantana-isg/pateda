...
#include "ProteinClass.h"  // for evaluating proteins   
...
{  
  ...
  //  cout<<" CliqMaxL "<<CliqMaxLength<<"  "<<MaxNumCliq<<"  "<<Complexity<<" "<<Prior<<"  "<<LearningType<<" "<<Cycles<<endl;
 ...
 while (i<Maxgen && BestEval<Max) //&& NPoints>10  
  {  
     ...
     MyMarkovNet->FindNeighbors(listclusters,CliqMaxLength,threshold); // The neighborhood is found from the matrix of mutual information
	  ...
	  /*   for(int ii=0;ii<vars;ii++) // The MOA structure is printed
	    {
	     cout<<"Struct "<<i<<" ";
             MyMarkovNet->SetOfCliques->ListCliques[ii]->print();    
            }
   ...
   if(printvals>1) 
    {     
     ...
     if(printvals)   cout<<"Gen : "<<i<<" Best: "<<BestEval<<" ProbBest: "<<auxprob<<" DifPoints: "<<NPoints<<" Nclusters "<<nclust<<endl;    
      ...
      if(LearningType== 7 || LearningType== 9) if(NPoints>10) IntTree->GenPop(Elit,pop);
...
int MNFDAlg(double Complexity, int typemodel, int sizecliq, int learntype)  //In this case, complexity is the threshold for chi-square 
{  
  int i,j, fgen,gap;  
  double auxprob,OldBest;     
  DynFDA* MyMarkovNet;  
...
{  
  ...
  switch(algtype)  
                     {                     
		       case 0: Markovinit(Complex,1,CliqMaxLength-1);break;  // Markov Network       1
		       ...
		       case 5: succ = MNFDAlg(Complex,1,CliqMaxLength-1,0);break; // MN-FDA illegal factorizations
                       case 6: succ = MNFDAlg(Complex,1,CliqMaxLength-1,4);  // MN-FDA legal factorizations 
                       case 7:   succ = MOA(Complex,2);break; // (MOA with Tree and time schedule)
                       case 8:   succ = MOA(Complex,2);break; // (MOA without time schedule )
                       case 9:   succ = MOA(Complex,2);break; // (MOA with tree and without time schedule )
...
int main( int argc, char *argv[] )
{
  ...
  // ./moacpp   1 4 1 30 500 15 20 1 10 3 8 0 0 1 2   // An example of calling MOA
  // ./moaprot 30 1 1 23 500 15 50 1 1000 3 53 0 1 1 2
  int i,a;
 ...
 //modeprotein = params[0];
 modeprotein = 2; 
if (modeprotein == 2) Card = 3;
 else if (modeprotein == 3) Card = 5;



 Tour = 0;                       // Truncation Selection is used
 //func = 8;                       // Index of the function, only for OchoaFun functions
 Ntrees = CliqMaxLength;                     // Number of Trees  for MT-EDA
 Elit = 1;                       // Elitism
...
double eval;
 ...
 if(ExperimentMode==4) LearningType = 6;
 else if (ExperimentMode==7) LearningType = 7;
 else if (ExperimentMode==8) LearningType = 8;
  else if (ExperimentMode==9) LearningType =9;


for(u=0;u<5000;u++) Cardinalities[u] = Card;  
fun = 53; //Function number for protein 

for(j=params[0];j<params[1];j++)
   {   
    protein_inst = j; //params[1]+j;
    ReadProtein(protein_inst,modeprotein);     
    
 cout<<"Alg : "<<ExperimentMode<<", number codifying bits : "<<length<<", n : "<<vars<<", psize : "<<psize<<", Trunc : "<<T<<", max-gen : "<<Maxgen<<", BestElit. : "<<BestElitism<<", NNeighbors  : "<<CliqMaxLength<<", MaxFun  : "<<Max<<", func : "<<func<<", params[0] : "<<modeprotein<<", params[1] : "<<j<<endl; 
