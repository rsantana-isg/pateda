  ...
  { 
	 ...
	 for (i=Marca;i<NumCliq;i++){ 
	       ...
	       newtablecliq->UpdateCliqFromConf(best);
               ...
               newtablecliq->Propagation(0); // Propagation using Max marginal

               newtablecliq->FindBestConf();           
        
               newtablecliq->HighestProb = KbestTrees[0]->Prob(newtablecliq->GetBestConf());
...
void JtPartition::Add(BinaryTreeModel* tree, float val) { 
...
while (  (i != -1) && (cant<howmany)) {   
    ...
    aPop->SetVal(i, KbestTrees[i]->GetMaxProb());
    i = ordering[i] ; 
    cant++; 
 } 
