"""
Run ONE weighted-probability pateda EDA on ONE PBO problem under ONE selection
method (single cluster process).

Companion of ``run_pbo_eda.py`` for ``compare_weighted_edas_pbo.py``.  Each
process handles one (algorithm, selection-method, function, dimension) tuple and
executes ``n_runs`` consecutive seeds (``seed, ..., seed+n_runs-1``) so that all
runs of that tuple land in the same IOH data folder.

Every process writes to its OWN folder (the IOH ``Analyzer`` logger cannot share
a folder between processes)::

    results/pbo_weighted_data_cluster/{ALG}__{SEL}_f{FID}_dim{DIM}_s{SEED}/
...
Usage (positional arguments, seed first):
    python3 scripts/run_weighted_pbo_eda.py seed n_runs alg sel fid dim pop_size n_gen [sel_ratio]

Example:
    python3 scripts/run_weighted_pbo_eda.py 1 5 EBNA RTS 19 100 200 50 0.5
"""
...
from compare_weighted_edas_pbo import (
...
OUTPUT_ROOT = os.path.abspath(
    os.path.join(SCRIPT_DIR, os.pardir, "results", "pbo_weighted_data_cluster")
)
