function [ListFactors,TheMax] = AllEvolveCorrelations(nfactors,ListOfLists,nsteps,tsign)
...
while(ll<nsteps)    
 ...
 s_ac1 = size(ac1,1);
  ...
  ListFactors{nf} = AuxListOfLists{nf};
  if (ll>1) & ( ((sig==1) & (TheMax(nf,ll-1)>=TheMax(nf,ll))) | ((sig==-1) & (TheMax(nf,ll-1)<=TheMax(nf,ll))))
    nf = mod((nf+1),nfactors)
  end  
  ll = ll + 1;
