...
{ 
  double prior; 
   ...
   prior  = (penalty*N)/(length*pow(2.0,dim-1.0)); 
   return prior;  
...
void BinaryTreeModel::CollectKConf(int K, int nvars,BinaryTreeModel* OrigTree,Popul *pop ) 
{
    JtPartition *treePart;
   BinaryTreeModel* MTree = new BinaryTreeModel(OrigTree,0); // Este arbol se libera en treePart
    MTree->Marca = 0;  
    MTree->FindRootCharges();
    MTree->NewPropagation(0);
    MTree->FindBestConf();
    MTree->HighestProb = OrigTree->Prob(MTree->GetBestConf(),0);
    //cout<<"init "<<MTree->HighestProb<<endl;
    treePart = new  JtPartition(K,nvars,nvars);
    treePart->SetInitTree(OrigTree);
    treePart->Add(MTree,MTree->HighestProb);
    treePart->Cycle();
    treePart->SetPop(pop,K);
    for (int l=0;l<treePart->GetCantConf()+1;l++)
      {  
	 for(int ll=0;ll<nvars;ll++) cout<<pop->P[l][ll]<<" "; 
	 cout<<"FirstVal "<<pop->GetVal(l)<<"  "<< OrigTree->Prob(pop->P[l],0);
         cout<<endl;
      }
    cout<<endl;
    delete treePart;  
...
{ 
  double prior; 
   ...
   prior  = (penalty*N)/(NumberVars*pow(2.0,dim-1.0)); 
   return prior;  
...
void BinaryTreeModel::CalProbFvect(Popul* EPop, double *vector,int howmany,int cprior) 
 { 
   ...
   //cout<<genepoollimit<<"  "<<actualpoolsize<<endl;
...
void BinaryTreeModel::CalMutInfDeception() 
 { 
   int j,k; 
	 // Segundo paso 
	 // Se construye un arbol que contendra un conjunto optimo 
	 ...
	 for(j=0; j<length-1; j++) 
	  	for(k=j+1 ; k<length; k++) 
			{ 
				...
				if (aux2 > 0.000000001) 
				 { 
				  auxuniv = ((1-AllProb[j])*(1-AllProb[k]));
			          MutualInf[aux]+=aux2*(log(aux2/auxuniv)); 
                                
                                  maxbiv =  aux2;
                                  maxuniv = auxuniv;
                                  bestufreq = 0;
                                  bestbfreq = 0;
                                  //cout<<j<<"  "<<k<<" "<<bestufreq<<" "<<bestbfreq<<" "<<maxbiv<<" "<<maxuniv<<endl;
				 }
...
void BinaryTreeModel::GenIndividual (Popul* NewPop, int pos) 
 { 
  // The vector in position pos is generated 
...
void JtPartition::Cycle() { 
 ...
 while ( (first<MaxKConf) && (firstindex != -1))    // El procedimiento termina cuando  
  ...
  { 
         ...
         BinaryTreeModel* BestTree= GetBestTree();  // Mejor conf. no visitada. 
	 best = BestTree->GetBestConf();    
         //cout<<" bestconf ";     
         //for (int k=0;k<NumVars;k++) cout<<best[k]<<" ";
         //cout<<"    "<<InitTree->Prob(BestTree->GetBestConf(),0)<<endl;
	 ...
	 for (i=Marca;i<NumCliq;i++){ 
	       ...
	       BinaryTreeModel* newtablecliq = new BinaryTreeModel(BestTree,i); // Se crean los descendientes 
               
	       newtablecliq->UpdateSecProb(BestTree,InitTree);
             
               newtablecliq->UpdateCliqFromConf(best);
		
               newtablecliq->NewPropagation(0); // Propagation using Max marginal

               newtablecliq->FindBestConf();           
        
               newtablecliq->HighestProb = InitTree->Prob(newtablecliq->GetBestConf(),0);
            
       	       double aux =    newtablecliq->GetMaxProb();    
                 
	       if (aux> 0 && newtablecliq->Prob(newtablecliq->GetBestConf(),0) > 0.0) 
	       {
		   //for (int k=0;k<NumVars;k++) cout<<newtablecliq->BestConf[k]<<" ";
		      //cout<<"  Marca "<<Marca<<" i "<<i<<" val "<<aux<<" valother "<<newtablecliq->Prob(newtablecliq->GetBestConf())<<endl;
                 Add(newtablecliq,aux);
               }
               else delete newtablecliq; 
   // De ser positiva se anhade a la lista 
...
JtPartition::JtPartition(int MaxConf, int CantVar, int CantCliq){ 
 ...
 KbestTrees  = new BinaryTreeModel*[MaxKConf]; 
...
void JtPartition::Add(BinaryTreeModel* tree, double val) { 
 ...
 F=0; //finit
  ...
  if (CantConf == MaxKConf-1){ 
    ...
    if ((val- KbestTrees[last]->GetMaxProb())<0.000000000001) delete tree; // Si es menor o igual que la peor no se incluye 
	 ...
	 F = FindFather(firstindex,last); 
	   ...
	   int AuxF = FindFather(0,CantConf); // MaxKConf 
           ...
           KbestTrees[CantConf] = (BinaryTreeModel*)0; 
	...
	while ((ordering[i] != -1) && ( (KbestTrees[i]->GetMaxProb()>val) || (fabs(KbestTrees[i]->GetMaxProb()-val)<0.000000000001) )) { 
  ...
  if (ordering[i] == -1){ 
	 ...
	 if ((val - KbestTrees[i]->GetMaxProb())>0.000000000001) { // El nuevo valor es > que el ultimo  
...
void JtPartition::SetPop(Popul* aPop, int howmany){ 
...
while (  (i != -1) && (cant<howmany)) {   
    best = KbestTrees[i]->GetBestConf(); 
    for (int j=0;j<NumVars;j++) aPop->P[cant][j]=best[j];  
    aPop->SetVal(cant, KbestTrees[i]->GetMaxProb());
    i = ordering[i] ; 
    cant++; 
 } 
...
while (i != -1) 
 ...
 void JtPartition::SetInitTree(BinaryTreeModel* ITree) 
  {
   InitTree =ITree;
  }
...
void IntTreeModel::CalProbFvect(Popul* EPop, double *vector,int howmany,int cprior, unsigned** matrix) 
 { 
    int j,k,l,ind_u,ind_b,aux;
    double tot;
     genepoollimit = howmany;
		  ...
		  if (matrix[j][k] ==1)
		   {
		    ...
		    {
                        for(l=0; l<genepoollimit; l++)  AllProb[ind_u+EPop->P[l][j]] += vector[l]; 
                    }                    
...
*/
 ...
 { 
   int j,k,l,m,aux; 
   double aux1,aux2; 
   double maxbiv,maxuniv_i,maxuniv_j,auxuniv;
   int bestufreq_i, bestufreq_j, bestbfreq_i, bestbfreq_j,malign,benign;
   ...
   benign = 0;
	       ...
	       aux = j*(2*length-j+1)/2 +k-2*j-1; 
	        ...
	        for(l=0; l<Card[j]; l++) 
                    ...
                    if(aux2 > maxbiv) 
		      {
                     ...
                     if(AllProb[IndexUnivEntries[j]]  > maxuniv_i) 
		      {
	       	       maxuniv_i = AllProb[IndexUnivEntries[j]];
                       bestufreq_i = l;
                       bestufreq_j = m;
                      }                    
                     if(AllProb[IndexUnivEntries[k]]  > maxuniv_j) 
		      {
                       maxuniv_j = AllProb[IndexUnivEntries[k]];
                       bestufreq_i = l;
                       bestufreq_j = m;
                      }		     
 		    ...
 		    if (aux1>0.000000001 &&  aux2 > 0.000000001) //0.000000001 
                   ...
                   if( (bestbfreq_i !=  bestufreq_i) || (bestbfreq_j !=  bestufreq_j) )
		      {    
                         malign ++;
                      }
                    else 
	...
	// cout<<j<<" "<<k<<" "<<"malign  "<<malign<<" benign "<<benign<<endl;	 
 }  
...
void IntTreeModel::IndividualMutation(Popul* NewPop,int MutType,int individual, int MutatedGenes) 
{ // Mutacion binaria 
  int i,pickedgene; 
	for(i=0; i<MutatedGenes; i++) 
	{ 
		pickedgene = randomint(NewPop->vars); 
                //cout<<"picked "<<pickedgene;
		NewPop->P[individual][pickedgene] = randomint(Card[pickedgene]); 
                //cout<<"Card "<<Card[pickedgene]<<" val  "<<NewPop->P[individual][pickedgene] ;
     } 
} 
	  ...
	  { 
		...
		{ 
		 ...
		 ind_b =  IndexBivEntries[aux];
               	   for(l=0; l<genepoollimit; l++) 
		    { 
		       AllSecProb[ind_b+Card[k]*EPop->P[l][j]+EPop->P[l][k]] += vector[l];   		       if (k==j+1) AllProb[ind_u+EPop->P[l][j]] += vector[l]; 
    		     }    		    
		} 
...
void LoopyIntTreeModel::CreateFields()  // Creates matrixs for TE13 potential function.
{  
  int j,k;
  
  order  = new int[length]; 
  Matrix = new unsigned int*[length];

      for(j=0;j<length;j++) 
      {
 	  order[j] = j+1;
          Matrix[j] =  new unsigned int[length];
...
void LoopyIntTreeModel::InitPsi()  
{  
   int j,k,m,l,aux; 
   EmptyPsi();  
	     ...
	     {               
	       if(Matrix[j][k]==1)
               {        
                 Psi[j][k] = new double[Card[j]*Card[k]];
                 Psi[k][j]  = Psi[j][k];
                 aux = j*(2*length-j+1)/2 +k-2*j-1; 
	         for(l=0; l<Card[j]; l++) 
   	          for(m=0; m<Card[k]; m++) 
	          { 
                    Psi[j][k][l*Card[k]+m] =  exp(-1.0/(1.0+AllSecProb[IndexBivEntries[aux]+l*Card[k]+m]));   
		    //cout<<j<<" "<<k<<" "<<l<<" "<<m<<" "<< Psi[j][k][l*Card[k]+m]<<endl; 
                  }
	       }
	     }	           			 
 }   
...
void LoopyIntTreeModel::FillPsi()  
{  
   int j,k,l,m,aux; 
          for(j=0; j<length-1; j++) 
   	   for(k=j+1 ; k<length; k++) 
...
void LoopyIntTreeModel::EmptyPsi()  
{ 
  int i,j;
  ...
  for(i=0;i<length;i++)
    {
      Psi[i] = new double*[length]; 
      for(j=0;j<length;j++)  Psi[i][j] = (double*)0;
    }

}


void LoopyIntTreeModel::InitMatrix(unsigned int** Mat)  
{                      
  int i,j;
...
void LoopyIntTreeModel::MatrixFromTree()  
{                      
  int i,j;
...
int LoopyIntTreeModel::GenMaxConfigurations(double temperature,int MaxIter, int psize, int Elit, Popul* pop)
{
  double** Lambda = 0; 
  int i,j;
  ...
  double* energies;
  int numberconf;
  unsigned int* ProvCard;
  ProvCard =  new unsigned int[length];

  for(i=0;i<length;i++)  ProvCard[i] = Card[i];

  InferenceClass*  PotentialInference  = new  InferenceClass(AT_LOOPY,GENERAL, length,Matrix,ProvCard,Ls,Psi,Lambda,temperature); 
  /*
  for(i=0;i<length;i++)
 {
   for(j=0;j<length;j++)
     {
       if(Matrix[i][j] == 1) cout<<"InitMatrix "<<i<<" "<<j<<endl; 
     }
 } 
  */  
        numberconf = psize;
         ...
         for(i=0;i<numberconf;i++)  bestconf[i] = new unsigned[length];
 ...
 for(i=0;i<aux;i++) 
    for(j=0;j<length;j++)
      pop->P[i+Elit][j] = bestconf[i][j];
 
 //if(aux<numberconf)
   {
     //  cout<<"Only "<<aux<<" of "<<numberconf<<" configurations found"<<endl;
     //delete PotentialInference;
     //  PotentialInference  = new  InferenceClass(AT_LOOPY,GENERAL, length,Matrix,ProvCard,Ls,Psi,Lambda,temperature); 
     //  PotentialInference->LoopyPropagation(MaxIter,MAX,SEQUENTIAL);
  
     /*
     for(j=0; j<length-1; j++) 
       for(l=0; l<Card[j]; l++) 
        AllProb[IndexUnivEntries[j]+l] =  PotentialInference->beliefs[i][l];
     ...
     for(j=0; j<length-1; j++) 
	   ...
	   n = -1;
	         ...
	         for(l=0; l<Card[j]; l++) 
		 {
   	          ...
   	          for(m=0; m<Card[k]; m++) 
	          { 
                    //cout<<j<<" "<<k<<" "<<n<<" "<<l<<" "<<m<<" "<<PotentialInference->pairBeliefs[j][n][l][m]<<endl;  
                    AllSecProb[IndexBivEntries[aux]+l*Card[k]+m] = PotentialInference->pairBeliefs[j][n][l][m];
   ...
   delete[] ProvCard;
   delete[] energies;
   for(i=0;i<numberconf;i++) delete[] bestconf[i];
   delete[] bestconf;
  
  //int aux  =  PotentialInference->MaxConfigurationsLoopy(bestconf, energies, PotentialInference, numberconf,MaxIter);
  
   //cout<<"Until here every thing "<<endl;
   delete PotentialInference;
   return aux;
 }
...
void LoopyIntTreeModel::InitPotential(unsigned int** Mat)
{
  CreateFields(); 
  InitMatrix(Mat);
  InitLs();
...
void LoopyIntTreeModel::DeletePotential()
{
  int j,k;
    ...
    for(j=0; j<length; j++) 
      {
	     ...
	     {               
	       if(Matrix[j][k]==1)
             ...
             delete[] Ls[j];     
             delete[] Psi[j];   
             if (Matrix[j] != (unsigned int*)0)   delete[] Matrix[j]; 
      }
     delete[] Ls;    
     delete[] Psi;    
...
LoopyIntTreeModel::LoopyIntTreeModel(int vars, double complexity,int apsize, unsigned int* Cards):IntTreeModel(vars,complexity,apsize,Cards) 
 { 
 } 


LoopyIntTreeModel::~LoopyIntTreeModel() 
 { 
 } 
