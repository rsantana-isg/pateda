...
double** GBP::inference(int* converged) {
  ...
  while (dBel>th && nIter<gbp_maxIter) {
    nIter++;
    ...
    new_beliefs = 0;
    //cout<<"nIter GBP "<<nIter<<" dBel "<<dBel<<" th "<<th<<" gbp_maxIter "<<gbp_maxIter<<endl;
  }

  if (nIter > gbp_maxIter) {
    (*converged) = -1;
    //mexPrintf("c-GBP: messages decreased to zero, iterating stopped\n");
    cout<<"c-GBP: messages decreased to zero, iterating stopped"<<endl;
  }
  else {
    if (dBel<=th) {
      (*converged) = nIter;
      ...
      //mexPrintf("c-GBP: did not converge after %d iterations\n",nIter);
       cout<<"c-GBP: did not converge after "<<nIter<<" iterations"<<endl;
    }
  }
  return ia_beliefs;
