...
int SNPs::CalculateNumberTaggedSNPsWithAdded(unsigned* solution, int howmany)
  {
    int i,j,k,covert,pick_one;
    unsigned* auxSNPs;
    int result;     
 ...
 i = 0;
  ...
  int alreadypicked = 0;
   ...
   while(covert<numberSNPs && alreadypicked<howmany)    //Those not covered yet are identified and randomly set to be tagging SNPs
    {
     pick_one = randomint(numberSNPs-covert)+1;
     ...
     // At this point i is the new tagging SNP
     auxSNPs[i-1] = 2;  
     solution[i-1] = 1; //The solution is modified
     covert++;
...
SNPSets::SNPSets(int nSNPSets,int indexSets)  
{    
  SetIdentifier = indexSets;
  numberSNPSets = nSNPSets;
  SelSNPSets =  new int[numberSNPSets];
  FillSetSNPSets(indexSets);
  TheSNPSets = new SNPs*[numberSNPSets];
  TheSNPIndex = new int*[numberSNPSets];
  BackSNPIndex = new int*[numberSNPSets];
  SNPsNames = new char*[MaxnumberSNPs];
  TheOptSNPIndex = new int[MaxnumberSNPs];
  OptBack = new int[MaxnumberSNPs];
...
{   
  ...
  delete[] BackSNPIndex;
  delete[] SelSNPSets;
  delete[] TheOptSNPIndex;
  delete[] OptBack;
}

void SNPSets::FillSetSNPSets(int indexSets)
{   
  int i,j,Found;
  numberOptSNPSets = 0;
  j = indexSets;
  //cout<<"Number of SNP Sets "<<numberSNPSets<<endl;
    for (i=0;i<numberSNPSets;i++)
    {
      if(j%2) SelSNPSets[numberSNPSets-i-1]=1;
      else  SelSNPSets[numberSNPSets-i-1]=0;
      numberOptSNPSets+=SelSNPSets[numberSNPSets-i-1];
      //  cout<<numberSNPSets-i-1<<" "<<j<<" -> "<<SelSNPSets[numberSNPSets-i-1]<<" ";
      j=j/2;
    }
    //   cout<<"   ::: "<<numberOptSNPSets<<endl;
}
  ...
  for (i=0;i<numberSNPSets;i++)
    {
      ...
      for (j=0;j<MaxnumberSNPs;j++) TheSNPIndex[i][j] = -1;
     
      BackSNPIndex[i] = new int[TheSNPSets[i]->numberSNPs];
      for (j=0;j<TheSNPSets[i]->numberSNPs;j++) BackSNPIndex[i][j] = -1;
      ...
      for (j=0;j<TheSNPSets[i]->numberSNPs;j++)
	{
      ...
      // The SNPs that will be used for optimization are selected
      // They are all those that are at least in one of the selected SNP populations
  ...
  for(j=0;j<numberSNPs;j++)
    {
     ...
     while(i<numberSNPSets && !found)
      {
       found = (SelSNPSets[i] && TheSNPIndex[i][j]>-1);
       i++;
      }

     if(found==1)
      {
   	 TheOptSNPIndex[numberOptSNPs]=j;
         OptBack[j] = numberOptSNPs;
         numberOptSNPs++;
      }
     //cout<<"j "<<j<<" i-1  "<<i-1<<"  "<<" SelSNPSetps "<<SelSNPSets[i-1]<<found<<"  "<<TheSNPIndex[i-1][j]<<endl;
    }
  //cout<<"numberOptSNPs "<<numberOptSNPs<<endl;
}
...
void SNPSets::OptCreateMatrix(unsigned** matrix)
  {
    // The matrix has numberSNPs^2 elements
    int i,j,k,l;
    int auxi, auxj, auxk, auxm;
 ...
 if(SelSNPSets[l]==1)
  {
   for (k=0;k<TheSNPSets[l]->numberSNPsNeedingTag;k++)
   {
     ...
     for (j=0; j<TheSNPSets[l]->ntagging[i];j++)
        {
          ...
          auxk = BackSNPIndex[l][i];
          auxk = OptBack[auxk];
          auxi = BackSNPIndex[l][TheSNPSets[l]->tagged[i][j]];
          auxi = OptBack[auxi];
          matrix[auxk][auxi] = 1;
          matrix[auxi][auxk] = 1;
	...
	{
	  auxj = BackSNPIndex[l][TheSNPSets[l]->tagged_by_pair[i][j]];
          auxj = OptBack[auxj];
          ...
          auxm = BackSNPIndex[l][TheSNPSets[l]->tagging_as_pair[i][j]]; 
          auxm = OptBack[auxm]; 
        
          matrix[auxm][auxj] = 1;
          matrix[auxj][auxm] = 1;                 
	} 
      //cout<<k<<" "<<i<<" "<<auxk<<" "<<auxi<<" "<<auxj<<" "<<auxm<<endl;
     }  	
    }
  }
 } 
...
void SNPSets::CreateMatrix(unsigned** matrix)
 ...
 void SNPSets::OptMultiCalculateNumberTaggedSNPs(unsigned* solution, double* Results, int AllOrSome)
  {
    int i,j,k,m;
    unsigned* auxsolution;
    int numberones,pick_one;
    double auxresult;
    int indSNPSet;

    //  AllOrSome=1 implies all populations are evaluated. 
    //  For AllOrSome=0 only populations selected by the code are evaluated

  numberones = 0;
  auxsolution = new unsigned[numberSNPs];      
  for(j=0;j<numberOptSNPs;j++) numberones+=solution[j];
  int auxcount = 0;
  //cout<<numberOptSNPs<<"  "<<numberSNPs<<endl;
  if(!AllOrSome)
    {
  if(myrand()>0.5) // One bit that improves the covering for each population is randomly set to one
    {
  for (i=0;i<numberSNPSets;i++)
   {
    if(SelSNPSets[i])
      {
       for(k=0;k<numberSNPs;k++) auxsolution[k]=0;
      
       for(k=0;k<numberOptSNPs;k++)
        { 
         m = TheOptSNPIndex[k]; // The opt index from the union
         j = TheSNPIndex[i][m]; 
         if (j!=-1) auxcount++;
         if(solution[k]==1 && j!=-1) auxsolution[j]=1;
         else auxsolution[j]=0;     
	 //cout<<m<<"  "<<j<<" "<<i<<endl;        
        }          
       for(k=0;k<TheSNPSets[i]->numberSNPs;k++)
        {
	 m = BackSNPIndex[i][k];
         j = OptBack[m];
         if(auxsolution[k]==1) solution[j]=1;             
        }             
 ...
 else // One bit for each population is randomly set to zero
 {
  for (i=0;i<numberSNPSets;i++)
   {       
    if(SelSNPSets[i])
      {
        pick_one = randomint(numberones);
        j = 0;
        k = 0;
        while(j<pick_one && k<numberOptSNPs)
        { 
	  if(solution[k]==1) j++;
          k++;        
        }  
  ...
  numberones = 0;
  for(j=0;j<numberOptSNPs;j++) numberones+=solution[j];
  Results[0] = numberOptSNPs-numberones;

   for (i=0;i<numberSNPSets;i++)
   {
    if(SelSNPSets[i] || AllOrSome)
      {
       for(k=0;k<numberSNPs;k++) auxsolution[k]=0;
	 ...
	 //cout<<m<<"  "<<j<<" "<<i<<endl;        
        ...
        auxresult = (1.0*TheSNPSets[i]->MultiCalculateNumberTaggedSNPs(auxsolution));       
        Results[indSNPSet+1] = auxresult/(1.0*TheSNPSets[i]->numberSNPs);
        //cout<<i<<" "<<numberones<<" "<<"  "<<auxresult<<" "<<Results[indSNPSet+1]<<endl;   
        indSNPSet++;  
      }
   }     
