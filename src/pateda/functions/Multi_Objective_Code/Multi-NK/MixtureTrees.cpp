...
void MixtureTrees::FindGammaValues()
{
	...
	//	for(i=0; i < NumberPoints; i++)  cout<<Probabilities[NumberTrees][i]<<" "; 
	// cout<<endl; 
     	...
     	for(i=0; i < NumberTrees; i++) 
	{
	 ...
	 // Equation Numb (3.17) in Melina's Thesis
	   //auxval = Probabilities[NumberTrees][j]*(NumberPoints*PopProb[j]);
          
	 if(Probabilities[NumberTrees][j]>0) 
          {
           auxval =  (Probabilities[i][j]/Probabilities[NumberTrees][j]);
           //cout<<i<<" "<<j<<" "<<PopProb[j]<<"  "<<Probabilities[NumberTrees][j]<<" "<<Probabilities[i][j]<<" "<<auxval<<endl; 
           Probabilities[i][j] = auxval;
          }
...
cout<<"Normalized"<<endl;
  ...
  for(i=0; i <NumberTrees+1; i++) 
	 ...
	 {
           cout<<Probabilities[i][j]<<" ";
           //cout<<Probabilities[NumberTrees][j]<<" ";
         }
	cout<<endl;
       }
  */
...
*/
	  ...
	  */
	  ...
	  //cout<<"Iter="<<Count<<" Likehood="<<LikehoodValues[Count]<<" KL="<<KL<<" Prob="<<MeanMixtProb<<" BestLikehood="<<BestLikehood<<endl;   
         UpdateTreesProb();	
...
{ 	
 int i,j;
    double auxval,auxval1,auxval2,sumprob,KL,KLExact;
    KL = 0;
    KLExact = 0;
	LikehoodValues[step] = 0;
        sumprob = 0;  
        ...
        for(j=0; j < CNumberPoints; j++)
	{
	  ...
	  {          
	   auxval  = (EveryTree[i]->Prob(AllPoints->Ind(j),0));
           ...
           auxval2 += Probabilities[i][j]*lambdavalues[i];
	 ...
	 // if (auxval1>0) LikehoodValues[step]  += (PopProb[j]*(log(PopProb[j])-log(auxval1)));     //else  LikehoodValues[step]  -= (CNumberPoints*PopProb[j]*log(PopProb[j]));   
        ...
        if (auxval1>0) 
             { 
               LikehoodValues[step]  += ((log(auxval1)));   
               sumprob += auxval1;
               KL += PopProb[j]*log(PopProb[j]/auxval1);  
               KLExact +=  PopProb[j]*log(PopProb[j]/auxval2);  
               //KLExact +=  PopProb[j]*(log(PopProb[j]/Probabilities[NumberTrees][j]));                 
	     }
	//cout<<" j "<<j<<"  RealProb "<<PopProb[j]<<" prob "<<auxval1<<endl;
         }
         MeanMixtProb = sumprob;
         cout<<step<<" "<<KL<<" "<<KLExact<<" "<<LikehoodValues[step]<<" "<<sumprob<<endl;
...
void MixtureTrees::SamplingFromMixture(Popul *FinalPoints)
{
   ...
   for(i=FinalPoints->elit; i<FinalPoints->psize; i++)
   {
     ...
     EveryTree[j]->GenIndividual(FinalPoints,i,1);
   } 
}
...
void MixtureTrees::FillProbabilities()
{
    ...
    for(i=0; i < NumberTrees; i++) 
	{
	 ...
	 {  
	  ...
	  TreeProb[i] += lambdaXprob;
	  //cout<<i<<"  "<<j<<"  "<<lambdaXprob<<"  "<<TreeProb[i]<<endl;
	  Probabilities[i][j] = NumberPoints*PopProb[j]*(lambdaXprob)  * lambdavalues[i];        
          Probabilities[NumberTrees][j] += (Probabilities[i][j]);
...
{  
    ...
    if (NewTree==1) MDL = Likehood - (complexcoef*log(1.0*CNumberPoints)*(MixtComplexity+NumberVars+1))/2;
    else MDL = Likehood - (complexcoef*log(1.0*CNumberPoints)*(MixtComplexity+1))/2; 
