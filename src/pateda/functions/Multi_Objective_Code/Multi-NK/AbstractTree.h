...
const int ForbidValue = -1; // Value -1 is not allowed for vector's variables 
 ...
 AbstractProbModel(int); 
   virtual ~AbstractProbModel(); 
    ...
    virtual void IndividualMutation(Popul*,int,int,int); 
...
class IntTreeModel:public AbstractTreeModel{ 
	 ...
	 virtual void CalMutInf(); 
         virtual void CalMutInfDeception(); 
         void CalMutInf(unsigned int** ); 
         virtual void GenIndividual (Popul*,int);
         virtual void GenPartialIndividual (Popul*,int,int,int*,int); 
	 virtual void CalProbFvect(Popul*,double*,int,int); 
         void CalProbFvect(Popul*,double*,int,int,unsigned**);   
         virtual void CalProbFvect(Popul*,double*,int); 
	 virtual void UpdateModel(double*,int,Popul*);
      	 virtual void UpdateModel(); 
         virtual void IndividualMutation(Popul*,int,int,int); 
...
class LoopyIntTreeModel:public IntTreeModel{ 
 public: 
     ...
     LoopyIntTreeModel(int,double,int,unsigned int*);
     ~LoopyIntTreeModel();
     void   CreateFields(); 
     void   InitPsi();  
     void   FillPsi();  
     void   InitLs();  
     void   EmptyPsi();  
     void   InitMatrix(unsigned int**);  
     int    GenMaxConfigurations(double,int, int,int, Popul*);
     void   InitPotential(unsigned int**); 
...
BinaryTreeModel   *InitTree; // Referencia al arbol inicial
 
int first, last, firstindex;  // Se utilizan para el trabajo con la lista ( ver funcion Cycle ) 
 
BinaryTreeModel   **KbestTrees;  // Conj. de configuraciones ( Arboles) 
   
JtPartition(int , int , int ); 
 
 inline BinaryTreeModel* GetBestTree(){return KbestTrees[firstindex];}; 
 
~JtPartition(); 
 
 void Add(BinaryTreeModel*, double);  // Adiciona una conf. a la lista 
 
 int FindFather(int , int ) ;    // Halla la conf. anterior a son en la lista ordenada 
 
 void SetPop(Popul* , int );   // Llena una poblacion a partir de la lista 
 ...
 inline int  GetCantConf() {return CantConf;}; //Devuelve el numero de configuraciones actual
  ...
  void SetInitTree(BinaryTreeModel*);
}; 
