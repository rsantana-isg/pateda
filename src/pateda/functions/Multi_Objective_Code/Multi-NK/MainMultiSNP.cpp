...
for(pos=start; pos < epsize;  pos ++)  
 {
   ...
   ContainerSNPSets->OptMultiCalculateNumberTaggedSNPs(epop->P[pos],CurrentEval,0);
...
void SaveBestSolutions(MultiPopul* epop, int epsize)
{
 double CurrentEval[30];
 int i,start,pos; 
 char FinalTagsFileName[60];
 char FinalValsFileName[60];
 char FinalSolsFileName[60];
 ...
 itoa (ContainerSNPSets-> SetIdentifier,buffer,10);

 FinalTagsFileName[0]=0;
 strcat(FinalTagsFileName,TagsFilename); 
 strcat(FinalTagsFileName,"Tags");  
strcat(FinalTagsFileName,buffer);  

 FinalValsFileName[0]=0;
 strcat(FinalValsFileName,TagsFilename); 
 strcat(FinalValsFileName,"Vals");  
strcat(FinalValsFileName,buffer);  

 FinalSolsFileName[0]=0;
 strcat(FinalSolsFileName,TagsFilename); 
 strcat(FinalSolsFileName,"Sols");  
strcat(FinalSolsFileName,buffer); 

 
 streamTags = fopen(FinalTagsFileName, "w+" );  
 streamVals = fopen(FinalValsFileName, "w+" );  
streamSols = fopen(FinalSolsFileName, "w+" );  

fprintf(streamTags, "NUMSOLS %d\n", epsize);
fprintf(streamVals, "%d\n", epsize);
fprintf(streamVals, "%d\n",ContainerSNPSets->numberOptSNPs);
fprintf(streamSols, "%d\n", epsize);
fprintf(streamSols, "%d\n",ContainerSNPSets->numberOptSNPs);

for(pos=0; pos < epsize;  pos ++)  
 {
  ContainerSNPSets->OptMultiCalculateNumberTaggedSNPs(epop->P[pos],CurrentEval,1);
    
  fprintf(streamTags, "NSNPS:: %f\n", (ContainerSNPSets->numberOptSNPs-CurrentEval[0]));
  for (i=0;i<ContainerSNPSets->numberOptSNPs;i++) 
   {  
     if(epop->P[pos][i]==1) fprintf(streamTags, "%s\n",ContainerSNPSets->SNPsNames[ContainerSNPSets->TheOptSNPIndex[i]]);      
     fprintf(streamSols, "%d ",epop->P[pos][i]);      
   }   

   fprintf(streamSols, "\n"); 
  //fprintf(streamVals, " %d",(ContainerSNPSets->numberOptSNPs-CurrentEval[0]));   
   for (i=0;i<ContainerSNPSets->numberSNPSets+1;i++) 
   {  
     fprintf(streamVals, " %f", CurrentEval[i]);        
   }   
   fprintf(streamVals,"\n"); 
 }
...
//if(printvals)   cout<<"Gen :"<<i<<" Best:"<<BestEval<<" DifPoints:"<<NPoints<<endl;    
 ...
 SaveBestSolutions(selpop,selpop->psize);
  DeletePopulations(); 
  delete IntTree;
  return fgen;  
...
char PairsFilename [80];
char TriplesFilename [80];
char MapFilename [80];
char HapFilename [80];
...
params = new int[3];  
 ...
 BestElitism = atoi(argv[9]);         // If there is or not BestElitism, if thereisnot BestElitism, Elitism = 1 by default;
 printvals =  atoi(argv[10]);
 srand(ta+atoi(argv[11]));

cout << "Parameters:"<<endl;
cout << "ConfFilename: " << ConfFilename << endl;
cout << "MAXD: " << MAXD << ". MINC: " << MINC << ". MINM: " << MINM << endl;
...
cout << "Seed: " << ta+atoi(argv[14]) << endl;
 ...
 stream = fopen(ConfFilename, "r+" );
 if( stream == NULL ){
   printf( "The %s filename is missing or not correct\n",ConfFilename);
  exit(1);
 }
 else{
   fscanf( stream, "%d\n", &numberSNPSets);
   ...
   ContainerSNPSets = new SNPSets(numberSNPSets,codeSNPSets);

   cout << "Datasets provided:  " << numberSNPSets << endl;
   cout << "-----------------------------" << endl;
   cout << "Code for Subset of Datasets used for optimization:  " << codeSNPSets << endl;
   cout << "-----------------------------" << endl;

   for (int i=0;i<numberSNPSets;i++){
     cout << "Dataset number: " << i+1 << endl;

     fscanf(stream,"%s",MapFilename);
     cout << "Leee" << endl;
     cout << "MapFilename: " << MapFilename << endl;

     fscanf(stream,"%s",&HapFilename);
     cout << "HapFilename: " << HapFilename << endl;
     fscanf(stream,"%s",&PairsFilename);
     cout << "TriplesFilename: " << PairsFilename << endl;
     fscanf(stream,"%s",&TriplesFilename);
     ...
     ContainerSNPSets->TheSNPSets[i] = AllSNPs;
 ...
 vars = ContainerSNPSets->numberOptSNPs;
 Max = ContainerSNPSets->numberOptSNPs;
 NObj = ContainerSNPSets->numberOptSNPSets+1; //The additional objective is the number of tags

 matrixSNPs = new unsigned*[vars];
 for (i=0;i<vars;i++) matrixSNPs[i] = new unsigned[vars];
 ContainerSNPSets->OptCreateMatrix(matrixSNPs);
