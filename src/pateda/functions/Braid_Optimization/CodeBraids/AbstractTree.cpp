...
void AbstractProbModel::PopulateFromOtherPop(int From, Popul* NewPop, Popul* OtherPop ) 
{  
  int i,j,ind; 
    for(i=From; i<NewPop->psize; i++) 
	{ 
          ind = randomint(From); 
          for(j=0; j<NewPop->vars; j++) NewPop->P[i][j] =  OtherPop->P[ind][j];
        }	
    //cout<<"pass by here"<<ind<<" "<<NewPop->vars<<endl; 
} 
...
void AbstractTreeModel::MakeTreeIndependentVars() 
 { 
   // Constructs a "tree" formed by independent variables (similar to UMDA) 
...
void AbstractTreeModel::MakeTreeMarkovChain() 
 { 
   // Constructs a "tree" formed by independent variables (similar to UMDA) 
...
void AbstractTreeModel::PrintModel() 
 { 
	...
	int j; 
	    ...
	    if(Tree[Queue[j]]!=-1) 
              {
                cout<<Queue[j]<<" "<<Tree[Queue[j]]<<" "; 
                if(abs(Queue[j]-Tree[Queue[j]])==1) acc+=1.0;
                else if(abs(Queue[j]-Tree[Queue[j]])==2) acc+=0.5;
                else if(abs(Queue[j]-Tree[Queue[j]])==3) acc+=0.25;
              }
	    else cout<<Queue[j]<<" "<<-1<<" "; 
          } 
        cout<<acc;
...
//Necesario incorporar los priors en el memset.
     ...
     memset(AllSecProb, 0, TotBivEntries*sizeof(double));
	  ...
	  { 
       ...
       //  for(j=0; j<TotUnivEntries; j++) cout<< AllProb[j]<< " ";
       // cout<<endl; 
/*
	for(j=0; j<length*(length+1)/2; j++)
         { 
...
void IntTreeModel::SampleSomeVariables(Popul* NewPop, int pos, double samplethreshold) 
 { 
  // The vector in position pos is generated 
...
void IntTreeModel::GenPopSomeVariables(int From, Popul* NewPop, double samplethreshold) 
 {  
 int i; 
    for(i=From; i<NewPop->psize; i++) 
	{ 
	  SampleSomeVariables(NewPop,i,samplethreshold);  
	} 
 } 
...
int IntTreeModel::FindRootNode() 
 { 
	 ...
	 // Siguiendo a De Bonet aqui se escoge el de menor entropia incondicional 
	// Se determina la variable con menor entropia incondicional 
        // PARECE SER VALIDO PARA VARIABLES BINARIAS
