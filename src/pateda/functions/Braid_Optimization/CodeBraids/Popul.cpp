  ...
  if(maxval>minval)
    {
      for(i=0;i<psize;i++)  
      {  
       pvect[i] = (Evaluations[i]-minval)/(maxval-minval);
       pvect[i] = pow(base,pvect[i]/temp); 
       Z += pvect[i];  
       //cout<<pvect[i]<<" ";
      } 
      //cout<<endl;
     for(i=0;i<psize;i++) pvect[i]/=Z; 
    }
  else 
    for(i=0;i<psize;i++)  pvect[i]=1/psize;
  //cout<<"Passed by here "<<Z<<endl;
} 
